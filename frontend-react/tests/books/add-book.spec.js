...
test.describe('Books - Add New Book', () => {
	...
	test('should enable button when both fields have content', async ({ page }) => {
		const book = TEST_BOOKS.book1;
	...
	test('should have correct input placeholders', async ({ page }) => {
		await expect(page.locator(SELECTORS.titleInput)).toHaveAttribute('placeholder', 'Enter book title...');
		await expect(page.locator(SELECTORS.authorInput)).toHaveAttribute('placeholder', 'Enter author name...');
	});

	test('should focus on title input after form submission', async ({ page }) => {
		await addBook(page, TEST_BOOKS.book1);
