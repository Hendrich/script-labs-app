...
test.describe("Books - Edit Book", () => {
  test.beforeEach(async ({ page }) => {
    ...
    // Pastikan form kosong sebelum menambah buku
  ...
  test("should apply edit mode styling", async ({ page }) => {
    const bookCard = page.locator(SELECTORS.bookCard).first();
    // Pastikan ada buku sebelum klik edit
    await expect(bookCard).toBeVisible({ timeout: 5000 });
    // Enter edit mode
    await bookCard.locator(SELECTORS.editButton).click();
  ...
  test("should handle multiple books in edit mode", async ({ page }) => {
    ...
    // Pastikan ada dua book card
    const bookCards = page.locator(SELECTORS.bookCard);
    await expect(bookCards).toHaveCount(2, { timeout: 5000 });
    const firstCard = bookCards.first();
    const secondCard = bookCards.last();
